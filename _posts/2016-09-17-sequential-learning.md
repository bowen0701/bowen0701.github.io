...
- There are $latex K$ slot machines, where each slot machine is also called an one-armed bandit.
- The setting proceeds over $latex T$ iterations; hence **sequential**.
...
The goal of the decision maker is to maximize the total reward over all $latex T$ arm pulls. The decision maker only learns about arms by pulling them; hence **partial information**.
