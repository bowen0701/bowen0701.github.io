...
Q: Why **Monte Carlo method** is often the default method for high-dimensional integral approximation, **instead of Quadrature methods?** I got this question while reading [Fearnhead et al. (2025), Scalable Monte Carlo for Bayesian Learning, p.6](https://arxiv.org/abs/2407.12751).
...
**Step 1:** Take absolute value of the remainder: $$|R_r(x)| = \left| \frac{1}{(r-1)!} \int_{x_0}^{x} (x-t)^{r-1} f^{(r)}(t) \, dt \right|$$**Step 2:** Apply triangle inequality: $$|R_r(x)| \leq \frac{1}{(r-1)!} \int_{x_0}^{x} |x-t|^{r-1} |f^{(r)}(t)| \, dt$$**Step 3:** Use $|f^{(r)}(t)| \leq |f^{(r)}|_{\infty}$ (definition of sup norm): $$|R_r(x)| \leq \frac{|f^{(r)}|_{\infty}}{(r-1)!} \int_{x_0}^{x} |x-t|^{r-1} \, dt$$**Step 4:** Evaluate the integral. Let $h = |x - x_0|$: $$\int_{x_0}^{x} |x-t|^{r-1} \, dt = \int_{0}^{h} s^{r-1} \, ds = \frac{h^r}{r}$$**Step 5:** Combine: $$|R_r(x)| \leq \frac{|f^{(r)}|_{\infty}}{(r-1)!} \cdot \frac{h^r}{r} = \frac{|f^{(r)}|_{\infty}}{r!} h^r$$**Step 6:** Set $\delta = h = |x - x_0|$ and $C = \frac{1}{r!}$: $$\boxed{|R_r(x)| \leq C \cdot \delta^r \cdot |f^{(r)}|_{\infty}}$$
...
So: $$f(x) = f(x_0) + f'(x_0)(x-x_0) + \int_{x_0}^{x} (x-t) f''(t) \, dt$$
...
So: $$f(x) = f(x_0) + f'(x_0)(x-x_0) + \frac{f''(x_0)}{2!}(x-x_0)^2 + \int_{x_0}^{x} \frac{(x-t)^2}{2!} f'''(t) \, dt$$
#### Pattern Recognition

After k integration by parts, we get: $$f(x) = \sum_{j=0}^{k-1} \frac{f^{(j)}(x_0)}{j!}(x-x_0)^j + \int_{x_0}^{x} \frac{(x-t)^{k-1}}{(k-1)!} f^{(k)}(t) \, dt$$
Setting k = r:
